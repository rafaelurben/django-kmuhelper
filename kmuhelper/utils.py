...
def draw_qr_invoice(c, bestellung):
    def debug(c):
...
def draw_header(c, bestellung):
    ...
    c.setFont("Helvetica", 10)
    c.drawString(56*mm, 220*mm, (bestellung.datum.strftime("%Y")+"-"+str(bestellung.pk).zfill(6)+(" (Online-Bestellung Nr. "+str(bestellung.woocommerceid)+")" if bestellung.woocommerceid else "")))
...
def pdf_rechnung(bestellung):
    ...
    # Swiss QR-Code
    draw_qr_invoice(c, bestellung)
    # Header
    draw_header(c, bestellung)
