...
class WooCommerce():
    ...
    def product_create(self, product):
        newproduct = Produkt.objects.create(
            ...
            aktion_preis = (product["sale_price"] if product["sale_price"] else None)
        )
    ...
    def product_import(self):
        excludeids = str([obj.woocommerceid for obj in Produkt.objects.all().exclude(woocommerceid=0)])[1:-1]
        wcapi = self.get_api()
        ...
        for page in range(2,int(r.headers['X-WP-TotalPages'])+1):
            productlist += wcapi.get("products?exclude="+excludeids+"&page="+str(page)).json()
        print("[KMUHelper] - Produkte von WooCommerce heruntergeladen: "+str(len(productlist)))
        print("[KMUHelper] - Produkte von WooCommerce importieren...")
        for product in productlist:
            self.product_create(product)
        print("[KMUHelper] - Produkte von WooCommerce importiert: "+str(len(productlist)))
        return len(productlist)

    @classmethod
    def product_bulk_update(self, products):
        print("[KMUHelper] - Produkte von WooCommerce aktualisieren: "+str(len(products)))
        successcount = 0
        errorcount = 0
        for product in products:
            if product.woocommerceid:
                ...
                errorcount += 1
        print("[KMUHelper] - Produkte von WooCommerce aktualisiert: "+str(successcount))
    ...
    def customer_import(self):
        excludeids = str([obj.woocommerceid for obj in Kunde.objects.all().exclude(woocommerceid=0)])[1:-1]
        wcapi = self.get_api()
        ...
        customerlist += r.json()
        for page in range(2,int(r.headers['X-WP-TotalPages'])+1):
            customerlist += wcapi.get("customers?exclude="+excludeids+"&page="+str(page)).json()
        print("[KMUHelper] - Kunden von WooCommerce heruntergeladen: "+str(len(customerlist)))
        print("[KMUHelper] - Kunden von WooCommerce importieren...")
        for customer in customerlist:
            self.customer_create(customer)
        print("[KMUHelper] - Kunden von WooCommerce importiert: "+str(len(customerlist)))
        return len(customerlist)

    @classmethod
    def customer_bulk_update(self, customers):
        print("[KMUHelper] - Kunden von WooCommerce aktualisieren: "+str(len(customers)))
        successcount = 0
        errorcount = 0
        for customer in customers:
            if customer.woocommerceid:
                ...
                errorcount += 1
        print("[KMUHelper] - Kunden von WooCommerce aktualisiert: "+str(successcount))
        return (successcount, errorcount)
    ...
    def category_update(self, category, newcategory=None):
        if not newcategory:
            newcategory = self.get_api().get("products/categories/"+str(category.woocommerceid)).json()
        category.name = preparestring(newcategory["name"])
        category.beschrieb = preparestring(newcategory["description"])
        category.bildlink = (newcategory["image"]["src"]) if newcategory["image"] else ""
        if newcategory["parent"]:
            tup = Kategorie.objects.get_or_create(woocommerceid=newcategory["parent"])
            if tup[1]:
                self.category_update(tup[0])
    ...
    def category_import(self):
        excludeids = str([obj.woocommerceid for obj in Kategorie.objects.all().exclude(woocommerceid=0)])[1:-1]
        wcapi = self.get_api()
        ...
        categorylist += r.json()
        for page in range(2,int(r.headers['X-WP-TotalPages'])+1):
            categorylist += wcapi.get("products/categories?exclude="+excludeids+"&page="+str(page)).json()
        print("[KMUHelper] - Kategorien von WooCommerce heruntergeladen: "+str(len(categorylist)))
        ...
        for category in categorylist:
            newcategory = Kategorie.objects.create(
                ...
                bildlink = (category["image"]["src"]) if category["image"] else "",
        ...
        for tup in categorieswithparents:
            tup[0].uebergeordnete_kategorie = Kategorie.objects.get(woocommerceid=tup[1])
            tup[0].save()
        print("[KMUHelper] - Kategorien von WooCommerce importiert: "+str(len(categorylist)))
        return len(categorylist)

    @classmethod
    def category_bulk_update(self, categories):
        print("[KMUHelper] - Kategorien von WooCommerce aktualisieren: "+str(len(categories)))
        successcount = 0
        errorcount = 0
        for category in categories:
            if category.woocommerceid:
                ...
                errorcount += 1
        print("[KMUHelper] - Kategorien von WooCommerce aktualisiert: "+str(successcount))
        return (successcount, errorcount)
    ...
    def order_create(self, order):
        ...
        if order["customer_id"]:
            kunde = Kunde.objects.get_or_create(woocommerceid=int(order["customer_id"]))
            if kunde[1]:
        ...
        for item in order["line_items"]:
            product = Produkt.objects.get_or_create(woocommerceid=int(item["product_id"]))
            if product[1]:
    ...
    def order_import(self):
        excludeids = str([obj.woocommerceid for obj in Bestellung.objects.all().exclude(woocommerceid=0)])[1:-1]
        wcapi = self.get_api()
        ...
        orderlist += r.json()
        for page in range(2,int(r.headers['X-WP-TotalPages'])+1):
            orderlist += wcapi.get("orders?exclude="+excludeids+"&page="+str(page)).json()
        print("[KMUHelper] - Bestellungen von WooCommerce heruntergeladen: "+str(len(orderlist)))
        print("[KMUHelper] - Bestellungen von WooCommerce importieren...")
        for order in orderlist:
            self.order_create(order)
        print("[KMUHelper] - Bestellungen von WooCommerce importiert: "+str(len(orderlist)))
        return len(orderlist)

    @classmethod
    def order_bulk_update(self, orders):
        print("[KMUHelper] - Kunden von WooCommerce aktualisieren: "+str(len(orders)))
        successcount = 0
        errorcount = 0
        for order in orders:
